...
export const getPublishedCreations = async(req, res)=>{
    try{
        ...
        const validCreations = (creations || []).filter((creation) => {
            const content = creation?.content
            return typeof content === 'string' && content.trim() !== '' && /^(https?:\/\/|data:image\/|blob:)/i.test(content)
        })

        res.json({ success: true, creations: validCreations })
    } catch(error){
         res.json({success: false, message: error.message})
    }
}
